...
<div x-cloak x-data="{ confirmId: null }" x-effect="document.body.classList.toggle('overflow-hidden', confirmId), document.body.classList.toggle('h-screen', confirmId)">
    ...
    <div x-show="confirmId === {{ $confirmId }}" x-transition.opacity class="fixed top-0 left-0 flex items-center justify-center w-full h-screen min-w-full bg-gray-500 bg-opacity-50 z-60" role="dialog" aria-modal="true">
        ...
        <div @click.outside="confirmId = null" @keydown.esc="confirmId = null" class="absolute flex flex-col w-full max-w-xs shadow-2xl z-70 bg-slate-50 rounded-xl">
            ...
            <button class="absolute top-0 right-0 p-1 text-gray-500 transition-all duration-300 rounded-full hover:text-gray-600 hover:bg-gray-100 focus:outline-none focus:shadow-outline" x-on:click="confirmId = null">
            ...
            <div class="flex items-center justify-center px-5 py-4 space-x-8">
                <button x-on:click="confirmId = null" class="px-5 py-2 font-semibold text-gray-600 transition duration-150 bg-white border border-gray-300 rounded-lg hover:bg-gray-300 hover:border-gray-300 hover:text-gray-900 focus:outline-none focus:shadow-outline">Cancel</button>

...
<div wire:ignore class="{{ $noMargin ? 'mb-0' : 'mb-5' }}">
	...
	toggleMenu(value) {
        ...
        <div class="flex items-center pr-4 border border-b-0 text-gray-400 border-gray-300 bg-gray-100 rounded-t-md">
            <div class="flex-1">
                <button type="button" class="inline-block px-4 py-2 font-semibold" :class="{ 'bg-gray-200': tab === 'write' }" x-on:click.prevent="tab = 'write'; showConvertedMarkdown = false">
                    <x-tabler-pencil class="w-5 h-5 text-gray-500 group-hover:text-gray-600 " /></button>
                <button type="button" class="inline-block px-4 py-2 font-semibold" :class="{ 'bg-gray-200': tab === 'preview' && showConvertedMarkdown === true }" x-on:click.prevent="tab = 'preview'; convertedMarkdown()">
                    <x-tabler-eye class="w-5 h-5 text-gray-500 group-hover:text-gray-600 " /></button>
            </div>
            @if ($toolbar)
            <button title="bold" x-tooltip="'bold'" type="button" class="inline-block px-2 py-2 group hover" x-on:click.prevent="toggleMenu('bold')">
                <x-tabler-bold class="w-5 h-5 text-gray-500 group-hover:text-gray-600" />
            </button>
            <button title="italic" x-tooltip="'italic'" type="button" class="inline-block px-2 py-2 group hover" x-on:click.prevent="toggleMenu('italic')">
                <x-tabler-italic class="w-5 h-5 text-gray-500 group-hover:text-gray-600" />
            </button>
            ...
            <button title="quote" x-tooltip="'quote'" type="button" class="inline-block px-2 py-2 group hover" x-on:click.prevent="toggleMenu('quote')">
                <x-tabler-quote class="w-5 h-5 text-gray-500 group-hover:text-gray-600 " />
            </button>
            <button title="link" x-tooltip="'link'" type="button" class="inline-block px-2 py-2 group hover" x-on:click.prevent="toggleMenu('link')">
                <x-tabler-link class="w-5 h-5 text-gray-500 group-hover:text-gray-600 " />
            </button>
            <button title="footnote" x-tooltip="'footnote'" type="button" class="inline-block px-2 py-2 group hover" x-on:click.prevent="toggleMenu('footnote')">
                <x-tabler-notes class="w-5 h-5 text-gray-500 group-hover:text-gray-600 " />
            </button>
            @endif
            ...
            <div class="relative" x-data="{ open: false }" x-on:click.away="open = false" x-on:close.stop="open = false">
                <button x-tooltip="'Markdown Cheatsheet'" type="button" class="inline-block px-2 py-2 rounded-lg group hover" x-on:click="open = ! open">
                    <x-tabler-info-circle class="w-5 h-5 text-gray-500 group-hover:text-gray-600 " />
                </button>
        ...
        <textarea spellcheck="false" x-show="! showConvertedMarkdown" id="{{ $id }}" x-ref="input" x-model="content" name="{{ $name }}" class="relative block w-full px-5 py-6 overflow-y-auto font-mono text-sm text-gray-700 transition duration-150 ease-in-out border resize-none bg-gray-50 focus:bg-white border-gray-300 form-textarea rounded-b-md focus:outline-none focus:border-gray-500 focus:ring-1 focus:ring-gray-500" :style="`height: ${height}; max-width: 100%`" data-enable-grammarly="false"></textarea>

        <div x-show="showConvertedMarkdown">
            <div x-html="convertedContent" class="w-full p-5 overflow-y-auto leading-6 prose bg-white border shadow-sm border-gray-300 max-w-none prose-indigo rounded-b-md" :style="`height: ${height}; max-width: 100%`"></div>
        </div>
    </div>
